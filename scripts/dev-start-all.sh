...
if is_port_listening "$RETAILER_PORT"; then
  ...
  (
    cd "$ROOT_DIR"
    "$ROOT_DIR/.venv/bin/python" retailer/scripts/seed_data.py
    exec "$ROOT_DIR/.venv/bin/python" -m retailer.cli serve --config retailer.json --port "$RETAILER_PORT"
  ) &
  retailer_pid=$!
...
echo "Retailer API docs:    http://localhost:$RETAILER_PORT/docs"
